...
export default function CreateExamPage() {
    ...
    const syncSchemaMatrix = async () => {
      try {
        ...
        if (
          ...
          Array.isArray(responseData.schema)
        ) {
        ...
        console.log(
          ...
          extractedSchema.length,
        );
      } catch (err) {
        console.error("Schema Fetch Error:", err);
        showNotification(
          "Failed to load exam categories from database. Please refresh and try again.",
          "error",
        );
      } finally {
  ...
  useEffect(() => {
    const totalTime = sections.reduce(
      (sum, currentSec) => sum + Number(currentSec.time || 0),
      0,
    );
        ...
        if (exam.problems && exam.problems.length > 0) {
          ...
          exam.problems.forEach((problem, pIdx) => {
            ...
            formattedQuestions.push({
              ...
              example_correct_option:
                problem.example?.correct_answer_index || 0,
            ...
            if (problem.sub_questions && problem.sub_questions.length > 0) {
              problem.sub_questions.forEach((sub) => {
                formattedQuestions.push({
                  ...
                  type: problem.section?.toLowerCase().includes("listen")
                    ? "listening"
                    : "mcq",
                  text: sub.text || "",
                  options: sub.options || ["", "", "", ""],
  ...
  const handleCategoryChange = (catId) => {
    ...
    const targetCluster = activeSchema.find(
      (c) => c.id === catId || c.category_name === catId,
    );
    ...
    if (
      targetCluster &&
      targetCluster.levels &&
      targetCluster.levels.length > 0
    ) {
    ...
    if (isJlpt) {
      ...
      setSections([
        { id: Date.now(), name: "General Section", questions: 0, time: "" },
      ]);
    }
  };
  ...
  const addSection = () =>
    setSections((p) => [
      ...p,
      { id: Date.now(), name: "New Section Block", questions: 0, time: 15 },
    ]);
  ...
  const addProblemContext = (sectionName) => {
    ...
    const sectionProblemsCount = questions.filter(
      (q) => q.section === sectionName && q.is_problem,
    ).length;
  ...
  const addStandardQuestion = (sectionName) => {
    ...
    const lastProblem = [...questions]
      .reverse()
      .find((q) => q.section === sectionName && q.is_problem);
    ...
    const currentQuestionsInSection = questions.filter(
      (q) => q.section === sectionName && !q.is_problem,
    ).length;
    ...
    if (
      ...
      currentQuestionsInSection >= Number(targetSection.questions)
    ) {
      showNotification(
        `⚠️ Maximum limit of ${targetSection.questions} questions reached for ${sectionName}.`,
        "error",
      );
      return;
    }
    ...
    setActiveQuestionId(newId);
    showNotification(
      `New ${isListening ? "Listening" : ""} question added to ${sectionName}!`,
    );
  };
  ...
  const addQuestionUnderProblem = (problemId) => {
    ...
    const currentQuestionsInSection = questions.filter(
      (q) => q.section === problem.section && !q.is_problem,
    ).length;
    ...
    if (
      ...
      showNotification(
        `⚠️ Maximum limit of ${targetSection.questions} questions reached for ${problem.section}.`,
        "error",
      );
      return;
    }
  ...
  const removeItem = (id) => {
    ...
    if (itemToRemove && itemToRemove.is_problem) {
      const linkedQuestions = questions.filter(
        (q) => q.parent_problem_id === id,
      );
      if (linkedQuestions.length > 0) {
        if (
          !window.confirm(
            `This will also delete ${linkedQuestions.length} question(s) linked to this problem. Continue?`,
          )
        ) {
          return;
        }
      ...
      const remaining = questions.filter(
        (q) => q.id !== id && q.parent_problem_id !== id,
      );
  ...
  const handlePublishExam = async (statusType) => {
    try {
      ...
      console.log(
        "📤 Sending exam payload:",
        JSON.stringify(examPayload, null, 2),
      );
      ...
      showNotification(
        err.message || "Failed to create exam. Please try again.",
        "error",
      );
    } finally {
        ...
        {toast.show && (
          <motion.div
            ...
            className={`fixed top-6 right-6 z-50 flex items-center gap-3 px-5 py-4 rounded-2xl shadow-2xl border backdrop-blur-xl max-w-sm ${
              toast.type === "success"
                ? "bg-[#0f1629] border-emerald-500/30 text-emerald-200"
                : "bg-[#0f1629] border-rose-500/30 text-rose-200"
            }`}
          >
            {toast.type === "success" ? (
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-xs mt-0.5">
              {examId
                ? "Modify your existing exam structure"
                : "Build and deploy your custom exam structure"}
            </p>
          </div>
          ...
          {STEPS.map((s, i) => (
            <div key={i} className="flex-1 flex items-center">
              ...
              >
                ...
                <span
                  className={`text-xs font-bold uppercase tracking-wider hidden md:block ${i === step ? "text-white" : i < step ? "text-emerald-400" : "text-gray-500"}`}
                >
                  {s}
                </span>
              </button>
              {i < STEPS.length - 1 && (
                <div
                  className={`flex-1 h-0.5 mx-3 transition-all ${i < step ? "bg-emerald-500" : "bg-white/5"}`}
                />
              )}
            </div>
          ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="flex flex-col gap-1.5">
                  ...
                  >
                    ...
                    {activeSchema &&
                      activeSchema.map((c) => (
                        <option
                          key={c.id || c.category_name}
                          value={c.id || c.category_name}
                          className="bg-[#070c19]"
                        >
                  ...
                  {!globalLoading && activeSchema.length === 0 && (
                    <p className="text-[9px] text-amber-400">
                      ⚠️ No active categories found in database
                    </p>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <select
                    ...
                    value={meta.level_id}
                    onChange={(e) =>
                      setMeta((p) => ({ ...p, level_id: e.target.value }))
                    }
                  ...
                  >
                    ...
                    {availableLevels.map((l) => (
                      <option
                        key={l.id || l.level_name}
                        value={l.id || l.level_name}
                        className="bg-[#070c19]"
                      >
                  ...
                  {availableLevels.length === 0 &&
                    ...
                    !globalLoading && (
                      <p className="text-[9px] text-amber-400">
                        ⚠️ No levels found for this category
                      </p>
                    )}
                  {availableLevels.length > 0 && (
                    <p className="text-[9px] text-slate-500">
                      {availableLevels.length} level(s) available
                    </p>
                  )}
                </div>
              </div>
                ...
                <label className="flex items-center gap-3 px-4 py-5 bg-white/5 border-2 border-dashed border-white/10 rounded-xl cursor-pointer hover:border-blue-500/30 transition-colors relative overflow-hidden h-20">
                  {meta.thumbnail ? (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/50">
                      ...
                      />
                      <div className="absolute bg-black/60 text-[10px] uppercase font-bold tracking-wider px-2 py-1 rounded-md text-white">
                        Change Image
                      </div>
                    </div>
                  ) : (
            ...
            <GlassCard className="p-6 space-y-5 bg-white/[0.01] border-white/5 shadow-2xl rounded-3xl">
              ...
              <div className="space-y-3">
                {sections.map((s) => (
                  <div
                    key={s.id}
                    className="flex items-center gap-3 p-4 bg-slate-950/40 rounded-2xl border border-white/5"
                  >
                    <GripVertical
                      size={16}
                    ...
                    <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div>
                        <label className="text-[10px] uppercase font-mono tracking-wider text-slate-500 mb-1 block">
                          Section Name
                        </label>
                        <Input
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          value={s.time}
                          onChange={(e) =>
                            updateSection(s.id, "time", e.target.value)
                          }
                        />
                      </div>
                    </div>
                    <button
                      ...
                      className="text-rose-400 hover:text-rose-300 p-1 flex-shrink-0 self-end mb-2"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
              ...
              <div className="p-4 bg-blue-500/5 border border-blue-500/15 rounded-2xl flex justify-between text-xs font-mono font-bold tracking-wide uppercase">
                <div>
                  ...
                  <span className="text-blue-400 ml-1">
                    {sections.reduce((s, r) => s + Number(r.questions || 0), 0)}
                  </span>
                </div>
                <div>
                  Total Time:{" "}
                  <span className="text-blue-400 ml-1">
                    {meta.duration_minutes} mins
                  </span>
                </div>
              </div>
            </GlassCard>
          ...
          {step === 2 && (
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
              ...
              <div className="lg:col-span-1 space-y-4 max-h-[600px] overflow-y-auto pr-2 scrollbar-thin">
                {sections.map((sec) => {
                  const secProblems = questions.filter(
                    (q) => q.section === sec.name && q.is_problem,
                  );
                  ...
                  const isFull =
                    maxQuestions > 0 && currentQuestions >= maxQuestions;

                  return (
                    <div
                      key={sec.id}
                      className="bg-slate-950/40 border border-white/5 rounded-2xl p-3"
                    >
                      ...
                      {isListening && (
                        <div className="mb-3 space-y-2">
                          <label
                            className={`flex items-center justify-center gap-2 px-3 py-2 border border-dashed rounded-lg cursor-pointer transition-colors text-[10px] uppercase font-bold tracking-wider ${
                              sec.audio_url
                          ...
                          >
                            ...
                            <input
                              ...
                              className="hidden"
                              onChange={(e) =>
                                handleSectionAudioUpload(sec.id, e)
                              }
                            />
                          </label>
                      ...
                      {secProblems.map((problem) => {
                        const childQuestions = getQuestionsForProblem(
                          problem.id,
                        );
                        return (
                          <div key={problem.id} className="space-y-1 mb-2">
                            <button
                              ...
                              className={`w-full text-left px-2.5 py-2 rounded-xl border transition-all text-[10px] font-semibold flex items-center justify-between ${
                                activeQuestionId === problem.id
                                  ? "border-blue-500 bg-blue-500/10 text-white"
                                  : "border-white/5 bg-slate-950/40 text-gray-400 hover:border-white/10"
                              }`}
                            >
                                  ...
                                  <button
                                    ...
                                    className={`w-full text-left px-2 py-1.5 rounded-lg border transition-all text-[9px] font-medium flex items-center justify-between ${
                                      activeQuestionId === q.id
                                        ...
                                        : "border-transparent bg-slate-950/30 text-gray-500 hover:border-white/10"
                                    }`}
                                  >
                                    ...
                                    {isListeningQ && q.image_url && (
                                      <span className="text-[8px] px-1 bg-purple-500/20 text-purple-300 rounded">
                                        🖼️
                                      </span>
                                    )}
                                  </button>
                          ...
                          return (
                            <button
                              ...
                              className={`w-full text-left px-2.5 py-2 rounded-xl border transition-all text-[10px] font-semibold flex items-center justify-between ${
                                activeQuestionId === q.id
                                  ? "border-emerald-500/50 bg-emerald-500/10 text-white"
                                  : "border-white/5 bg-slate-950/30 text-gray-500 hover:border-white/10"
                              }`}
                            >
                              <div className="flex items-center gap-1.5 truncate">
                                {isListeningQ ? (
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/5 mt-2">
                        ...
                        <button
                          onClick={() => {
                            if (isFull) {
                              showNotification(
                                `⚠️ Maximum limit of ${maxQuestions} questions reached for ${sec.name}.`,
                                "error",
                              );
                              return;
                            }
                        ...
                        <div className="mt-2">
                          <div className="w-full bg-slate-700/30 rounded-full h-1">
                            <div
                              ...
                              style={{
                                width: `${Math.min((currentQuestions / maxQuestions) * 100, 100)}%`,
                              }}
                            />
                          </div>
                ...
                {activeItem ? (
                  <GlassCard className="p-6 space-y-5 bg-white/[0.01] border-white/5 shadow-2xl rounded-3xl relative">
                    <div className="flex items-center justify-between border-b border-white/5 pb-3">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {!activeItem.is_problem &&
                          activeItem.parent_problem_id && (
                            <Badge color="purple" className="text-[9px]">
                              Linked to:{" "}
                              {questions.find(
                                (q) => q.id === activeItem.parent_problem_id,
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-3 gap-3">
                          ...
                          <div className="col-span-1">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-gray-400 block mb-1">
                              Questions:{" "}
                              {getQuestionsForProblem(activeItem.id).length}
                            </label>
                          </div>
                        </div>
                        ...
                        <div className="p-4 bg-slate-950/30 border border-white/5 rounded-2xl space-y-4">
                          <h4 className="text-xs font-bold uppercase tracking-wider text-blue-400">
                            Example Question Configuration (Optional)
                          </h4>
                            ...
                            <div className="space-y-2">
                              ...
                              <label className="flex items-center gap-3 px-4 py-3 bg-white/5 border-2 border-dashed border-purple-500/30 rounded-xl cursor-pointer hover:border-purple-500/50 transition-colors relative overflow-hidden h-24">
                                {activeItem.example_image_url ? (
                                  <div className="absolute inset-0 flex items-center justify-center bg-black/50">
                                    <img
                                      src={activeItem.example_image_url}
                                      alt="Example"
                                  ...
                                  <div className="flex flex-col items-center gap-1 w-full">
                                    ...
                                    <span className="text-[8px] text-gray-500">
                                      (Image should have 1, 2, 3, 4 marked)
                                    </span>
                                  </div>
                                )}
                                <input
                                  ...
                                  className="hidden"
                                  onChange={(e) =>
                                    handleExampleImageUpload(activeItem.id, e)
                                  }
                                />
                              </label>
                            </div>
                          ...
                          {activeItem.section
                            ...
                            <div className="space-y-2">
                              ...
                              <label
                                className={`flex items-center justify-center gap-2 px-4 py-2 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${
                                  activeItem.example_audio_url
                                    ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400"
                                    : "bg-white/5 border-purple-500/30 text-slate-400 hover:border-purple-500/50"
                                }`}
                              >
                                <Mic size={16} />
                                {activeItem.example_audio_url
                                ...
                                <input
                                  ...
                                  className="hidden"
                                  onChange={(e) =>
                                    handleExampleAudioUpload(activeItem.id, e)
                                  }
                                />
                              </label>
                        ...
                        {getQuestionsForProblem(activeItem.id).length > 0 && (
                          <div className="p-3 bg-slate-950/30 border border-white/5 rounded-xl">
                            ...
                            <div className="space-y-1 max-h-32 overflow-y-auto">
                              {getQuestionsForProblem(activeItem.id).map(
                                (q) => {
                                  const sectionQuestions =
                                    getQuestionsForSection(q.section);
                                  const globalIndex =
                                  ...
                                  return (
                                    <div
                                      key={q.id}
                                      className="flex items-center justify-between text-[10px] text-slate-300 px-2 py-1 bg-slate-950/40 rounded-lg"
                                    >
                                      <span>
                                        Q{questionNumber}: {q.text || "Empty"}
                                      </span>
                            ...
                            <label className="flex items-center gap-3 px-4 py-3 bg-white/5 border-2 border-dashed border-purple-500/30 rounded-xl cursor-pointer hover:border-purple-500/50 transition-colors relative overflow-hidden h-32">
                              {activeItem.image_url ? (
                                ...
                                <div className="flex flex-col items-center gap-2 w-full">
                                  ...
                                  />
                                  <span className="text-xs text-gray-400">
                                    Click to upload listening image
                                  </span>
                                  <span className="text-[8px] text-gray-500">
                                    (Image should have 1, 2, 3, 4 marked)
                                  </span>
                              ...
                              <input
                                ...
                                className="hidden"
                                onChange={(e) =>
                                  handleListeningImageUpload(activeItem.id, e)
                                }
                              />
                            </label>
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-slate-500 min-h-[300px]">
                    ...
                    <p className="text-xs opacity-60">
                      Add a Problem or Question from the sidebar
                    </p>
                  </div>
                )}
              </div>
          ...
          {step === 3 && (
            <div className="space-y-6">
              <GlassCard className="p-6 bg-white/[0.01] border-white/5 shadow-2xl rounded-3xl space-y-6">
                <h2 className="text-base font-bold uppercase tracking-wider border-b border-white/5 pb-2">
                  Final Review
                </h2>
                {error && (
                  <div className="bg-rose-500/5 border border-rose-500/20 text-rose-400 p-3 rounded-xl flex items-center gap-2 text-xs font-semibold">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    { label: "Exam Title", value: meta.title || "Untitled" },
                    {
                      label: "Category",
                      value: meta.category_id
                        ? meta.category_id.toUpperCase()
                        : "Not selected",
                    },
                    {
                      label: "Duration",
                      value: `${meta.duration_minutes} mins`,
                    },
                    {
                      label: "Level",
                      value: meta.level_id
                        ? meta.level_id.toUpperCase()
                        : "Not selected",
                    },
                  ].map((s, i) => (
                    <div
                      key={i}
                      className="p-4 bg-slate-950/40 border border-white/5 rounded-2xl shadow-md"
                    >
                ...
                <div className="space-y-2">
                  ...
                  {sections.map((s, i) => {
                    ...
                    const isComplete =
                      maxQuestions > 0 &&
                      sectionQuestions.length >= maxQuestions;

                    return (
                      <div
                        key={i}
                        className="flex justify-between items-center px-4 py-3 bg-slate-900/20 border border-white/5 rounded-xl text-xs font-medium"
                      >
                        ...
                        <div className="flex gap-4 text-slate-500 font-mono">
                          ...
                          >
                            {sectionQuestions.length} / {maxQuestions || 0}{" "}
                            Questions
                          </span>
                ...
                <div className="text-xs text-slate-500">
                  <p>
                    Total Problems:{" "}
                    {questions.filter((q) => q.is_problem).length}
                  </p>
                  <p>
                    Total Questions:{" "}
                    {questions.filter((q) => !q.is_problem).length}
