...
import { ShieldCheck, BookOpen, Eye, CheckCircle, AlertTriangle, X, Clock, Check, AlertCircle } from "lucide-react";
...
const ExamQualityAuditsPage = () => {
  ...
  const [confirmRejectModal, setConfirmRejectModal] = useState({ show: false, examId: null });

  // Simulate realistic network connection delay on component mounting
  useEffect(() => {
  ...
  const handleAuditDecision = (examId, newStatus) => {
    ...
    setTimeout(() => {
      ...
      if (newStatus === "published") {
        showNotification("Exam package passed structural audit. Material published live.", "success");
      } else {
        showNotification("Exam package rejected. Status updated to draft mode.", "error");
      }
    }, 800);
  };
  ...
  if (loading) {
    return (
      /* --- PREMIUM GRID SHIMMER SKELETON LOADERS --- */
      <div className="space-y-8 bg-[#060d1f] min-h-screen p-6 sm:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map(n => (
            <div key={n} className="h-56 bg-white/[0.02] border border-white/5 rounded-2xl animate-pulse p-5 flex flex-col justify-between">
              <div className="space-y-3">
                <div className="flex justify-between"><div className="w-16 h-5 bg-white/10 rounded-lg"/> <div className="w-12 h-4 bg-white/5 rounded"/></div>
                <div className="w-3/4 h-6 bg-white/10 rounded-lg"/>
                <div className="w-1/2 h-4 bg-white/5 rounded"/>
              </div>
              <div className="space-y-2">
                <div className="h-8 bg-white/5 rounded-xl w-full"/>
                <div className="grid grid-cols-2 gap-2"><div className="h-8 bg-white/5 rounded-xl"/><div className="h-8 bg-white/5 rounded-xl"/></div>
              </div>
            </div>
          ))}
        </div>
        ...
        {toast.show && (
          ...
          >
            ...
            <div className="flex-1">
              <p className="text-xs font-bold uppercase tracking-wider opacity-60">Audit Guard</p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            ...
            >
              <div className="flex items-center gap-3 text-rose-400 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-bold text-white">Reject Package</h3>
                  <p className="text-xs text-gray-400">Quality Check Intervention</p>
                </div>
              </div>

              <p className="text-sm text-gray-300 mb-6 leading-relaxed">
                Are you sure you want to fail the quality compliance check for this exam pack? It will be reverted back to the author's drafting workspace.
              </p>
              ...
              <div className="flex justify-end gap-2">
                <Button 
                  ...
                  onClick={() => setConfirmRejectModal({ show: false, examId: null })}
                >
                  Cancel
                </Button>
                <button
                  type="button"
                  onClick={() => handleAuditDecision(confirmRejectModal.examId, "draft")}
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-400 mt-1 text-sm">Verify exam structure, accuracy, and JLPT standard compliance before publication</p>
          </div>
          {exams.length > 0 && (
            <div className="flex items-center gap-2 px-4 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-xl self-start sm:self-center">
              ...
              <span className="text-emerald-300 text-xs font-semibold uppercase tracking-wider">{exams.length} Pending Packages</span>
            </div>
          )}
        </div>
      </motion.div>
        ...
        {exams.length === 0 ? (
          <div className="col-span-full">
            <div className="p-12 text-center bg-white/[0.01] border border-dashed border-white/5 rounded-2xl flex flex-col items-center justify-center text-gray-500">
              <CheckCircle size={32} className="text-emerald-500 mb-2 animate-pulse" />
              <p className="text-gray-300 font-medium text-sm">No Exam Packages in Quality Queue</p>
              <p className="text-xs text-gray-500 mt-1">All submitted evaluation materials have been verified cleanly.</p>
            </div>
          </div>
        ) : (
          exams.map((exam, idx) => (
              ...
              <GlassCard className="p-5 flex flex-col justify-between h-full border-white/5 bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/15 transition-all">
                <div>
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className="text-blue-400 font-extrabold text-sm tracking-wide">
                      {exam.price > 0 ? `LKR ${exam.price.toLocaleString()}` : "Free Tier"}
                    </span>
                  </div>
                  <h3 className="text-lg font-bold text-white mb-2 line-clamp-1 capitalize">{exam.title}</h3>
                  <div className="space-y-1 pt-1 mb-5">
                    <div className="flex items-center gap-2 text-xs text-gray-400">
                      <BookOpen size={13} className="text-blue-500/70" />
                      <span>{exam.questions?.length || 0} Dynamic Questions Enclosed</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs text-gray-500 font-mono">
                      <Clock size={13} className="text-gray-600" />
                      <span>Submitted: {new Date(exam.createdAt?.seconds * 1000).toLocaleDateString()}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-white/5 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      {actionLoading === exam.id ? "..." : <><Check size={12}/> Publish</>}
                    </Button>
                    <Button 
                      variant="danger" 
                      size="sm" 
                    ...
                    >
                      {actionLoading === exam.id ? "..." : <><X size={12}/> Reject</>}
                    </Button>
                  </div>
                </div>
              </GlassCard>
            ...
            >
              ...
              <div className="p-4 border-b border-white/10 flex justify-between items-center bg-black/20">
                <div className="flex items-center gap-3 text-blue-400">
                  <ShieldCheck size={18} />
                  <div>
                    <h3 className="font-bold text-white text-sm">Auditing Package: {selectedExam.title}</h3>
                    <p className="text-[10px] text-gray-500 font-mono uppercase tracking-widest mt-0.5">Structural Integrity Check In-Progress</p>
                  </div>
                </div>
                <button 
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-black/10 scrollbar-thin">
                {selectedExam.questions && selectedExam.questions.length > 0 ? (
                  selectedExam.questions.map((q, qIdx) => (
                    <div key={qIdx} className="p-5 bg-white/[0.02] border border-white/5 rounded-2xl space-y-4">
                      <div className="flex items-start gap-3">
                        ...
                        <p className="text-white text-sm font-semibold leading-relaxed pt-0.5">{q.questionText}</p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 pl-9 select-none">
                        {q.options?.map((opt, oIdx) => (
                          ...
                          >
                            <span><span className="opacity-40 mr-2 font-mono">{String.fromCharCode(65 + oIdx)}.</span> {opt}</span>
                            {oIdx === q.correctOptionIndex && <CheckCircle size={12} className="text-emerald-400 flex-shrink-0" />}
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center space-y-2 py-12">
                    ...
                    <p className="text-gray-200 font-bold text-sm">Critical Structural Error Detected</p>
                    <p className="text-xs text-gray-500 max-w-xs leading-normal">This package metadata contains zero valid options or schema parameters. Please reject immediately.</p>
                  </div>
                )}
              </div>
              ...
              <div className="p-4 border-t border-white/10 bg-black/20 flex justify-between items-center">
                <Button variant="ghost" size="sm" onClick={() => setSelectedExam(null)} className="text-xs border border-white/10 text-gray-400">
                  Exit Review
                </Button>
